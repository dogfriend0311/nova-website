...
// ─── Game Detail Page ──────────────────────────────────────────────────────────
export default function GameDetailPage({ gameId, sport, navigate }) {
  const mob = useIsMobile();
  const [game, setGame] = useState(null);
  const [loading, setLoading] = useState(true);
  const [pbp, setPbp] = useState([]);
  const [pbpLoading, setPbpLoading] = useState(false);
  const [pbpTab, setPbpTab] = useState("pbp");
  const [pbpInning, setPbpInning] = useState(null);
  const [rawAtBats, setRawAtBats] = useState([]);
    ...
    try {
      if (sport === "mlb") {
        ...
        if (!mlbData) {
          try {
            ...
            if (eR.ok) {
              ...
              const extId = eD?.header?.competitions?.[0]?.externalIds?.find(x => x.provider === "mlbgamepk" || x.provider === "gamepk" || x.provider === "mlb");
        ...
        if (mlbData) {
          ...
          setRawAtBats(mapped.slice().reverse());
        ...
        if (r.ok) {
          ...
          const mapped = raw.length > 0 ? raw.slice().reverse().map((p, i) => ({
            ...
            awayScore: p.awayScore ?? null,
            homeScore: p.homeScore ?? null,
            isScoring: !!(p.scoringPlay || p.type?.text === "Goal" || p.type?.id === "goal" || (p.text || "").toLowerCase().includes("goal") || (p.type?.description || "").toLowerCase().includes("goal")),
            type: p.type?.text || p.type?.displayName || "",
          })) : [];
  ...
  useEffect(() => {
    loadPbp();
    const interval = sport === "mlb" || sport === "nba" || sport === "nhl" ? setInterval(() => loadPbp(), 20000) : null;
          ...
          <Card style={{ padding: "16px 18px" }} hover={false}>
            <div style={{ display: "flex", gap: 6, marginBottom: 12, alignItems: "center", justifyContent: "space-between", flexWrap: "wrap" }}>
              <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                {[["pbp", sport === "mlb" ? "⚾ At-Bats" : sport === "nhl" ? "🏒 All Plays" : "🏀 All Plays"],
                  ...
                  <button key={t} onClick={() => setPbpTab(t)} style={{ padding: "5px 12px", borderRadius: 14, cursor: "pointer", fontSize: 10, fontFamily: "'Orbitron', sans-serif", fontWeight: 700, border: `1px solid ${pbpTab === t ? "rgba(34,197,94,.45)" : "rgba(255,255,255,.1)"}`, background: pbpTab === t ? "rgba(34,197,94,.1)" : "rgba(255,255,255,.03)", color: pbpTab === t ? "#22C55E" : "#64748B" }}>{l}</button>
                ))}
              </div>
            ...
            {pbpPeriods.length > 1 && (
              <div style={{ display: "flex", gap: 5, marginBottom: 12, overflowX: "auto", paddingBottom: 4 }}>
                <button onClick={() => setPbpInning(null)} style={{ padding: "4px 10px", borderRadius: 12, cursor: "pointer", fontSize: 9, fontFamily: "'Orbitron', sans-serif", fontWeight: 700, flexShrink: 0, border: `1px solid ${pbpInning === null ? "rgba(0,212,255,.4)" : "rgba(255,255,255,.1)"}`, background: pbpInning === null ? "rgba(0,212,255,.1)" : "rgba(255,255,255,.03)", color: pbpInning === null ? "#00D4FF" : "#475569" }}>All</button>
                {pbpPeriods.map(p => (
                  <button key={p} onClick={() => setPbpInning(p)} style={{ padding: "4px 10px", borderRadius: 12, cursor: "pointer", fontSize: 9, fontFamily: "'Orbitron', sans-serif", fontWeight: 700, flexShrink: 0, border: `1px solid ${pbpInning === p ? "rgba(0,212,255,.4)" : "rgba(255,255,255,.1)"}`, background: pbpInning === p ? "rgba(0,212,255,.1)" : "rgba(255,255,255,.03)", color: pbpInning === p ? "#00D4FF" : "#475569" }}>
                      ...
                      {pitches.length > 0 && (
                        <div style={{ padding: "8px 12px", display: "flex", gap: 6, alignItems: "flex-start", flexWrap: "wrap" }}>
                          <div style={{ position: "relative", width: 80, height: 90, flexShrink: 0 }}>
                            <div style={{ position: "absolute", left: 14, top: 8, width: 52, height: 65, border: "1px solid rgba(255,255,255,.2)", borderRadius: 2, background: "rgba(255,255,255,.02)" }}>
                            ...
                            {pitches.map((p, pi) => {
                              ...
                              return (
                                <div key={pi} title={`${p.type} ${p.velocity ? p.velocity + "mph" : ""} — ${p.call}`}
                                  style={{ position: "absolute", width: isLast ? 10 : 8, height: isLast ? 10 : 8, borderRadius: "50%", background: col, left: x - 4, top: y - 4, border: isLast ? "2px solid rgba(255,255,255,.8)" : "1px solid rgba(0,0,0,.4)", fontSize: 6, display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(0,0,0,.7)", fontWeight: 900, zIndex: pi + 1, boxShadow: isLast ? `0 0 6px ${col}` : undefined }}>
                                  {pi + 1}
                                </div>
                              );
                            })}
                          ...
                          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 3, minWidth: 0 }}>
                            {pitches.map((p, pi) => {
                              ...
                              return (
                                <div key={pi} style={{ display: "flex", gap: 5, alignItems: "center", fontSize: 10, padding: "2px 0", borderBottom: pi < pitches.length - 1 ? "1px solid rgba(255,255,255,.04)" : "none" }}>
            ...
            {sport === "nba" && pbpFiltered.length > 0 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 460, overflowY: "auto" }}>
                {pbpFiltered.map((play, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: "flex", gap: 10, padding: "7px 10px", borderRadius: 7, background: isCurrent ? "rgba(59,130,246,.07)" : isScore ? "rgba(34,197,94,.05)" : "rgba(255,255,255,.02)", border: `1px solid ${isCurrent ? "rgba(59,130,246,.25)" : isScore ? "rgba(34,197,94,.15)" : "rgba(255,255,255,.05)"}`, alignItems: "center" }}>
            ...
            {sport === "nhl" && pbpFiltered.length > 0 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 460, overflowY: "auto" }}>
                {pbpFiltered.map((play, i) => {
                  ...
                  const score = play.awayScore != null ? `${play.awayScore}–${play.homeScore}` : "";
                  return (
                    <div key={i} style={{ display: "flex", gap: 10, padding: "7px 10px", borderRadius: 7, background: isScore ? "rgba(34,197,94,.07)" : "rgba(255,255,255,.02)", border: `1px solid ${isScore ? "rgba(34,197,94,.25)" : "rgba(255,255,255,.06)"}`, alignItems: "center" }}>
                      <div style={{ minWidth: 46, flexShrink: 0 }}>
                      ...
                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11, minWidth: Math.max(360, labels.length * 52 + 140) }}>
                        ...
                        <tbody>
                          {players.filter(Boolean).map((p, pi) => {
                            ...
                            return (
                              <tr key={pi} style={{ background: isTop ? "rgba(245,158,11,.04)" : pi % 2 === 0 ? "rgba(255,255,255,.02)" : "transparent", borderBottom: "1px solid rgba(255,255,255,.04)" }}>
                                ...
                                {stats.map((s, si2) => {
                                  ...
                                  return (
                                    <td key={si2} style={{ padding: "6px 6px", textAlign: "center", color: isKey && !isZero ? "#E2E8F0" : "#64748B", fontWeight: isKey && !isZero ? 700 : 400, fontSize: isKey ? 12 : 11, fontFamily: isKey ? "'Orbitron', sans-serif" : "'Rajdhani', sans-serif" }}>{val}</td>
                                  );
                                })}
                              </tr>
                            );
