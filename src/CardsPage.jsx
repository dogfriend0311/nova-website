...
const MLB_TEAMS_LIST = [
  { id:"108", name:"Los Angeles Angels",      abbr:"LAA", emoji:"😇" }, { id:"109", name:"Arizona Diamondbacks",    abbr:"ARI", emoji:"🐍" },
  { id:"110", name:"Baltimore Orioles",       abbr:"BAL", emoji:"🐦" }, { id:"111", name:"Boston Red Sox",          abbr:"BOS", emoji:"🧦" },
  { id:"112", name:"Chicago Cubs",            abbr:"CHC", emoji:"🐻" }, { id:"113", name:"Cincinnati Reds",         abbr:"CIN", emoji:"🔴" },
  { id:"114", name:"Cleveland Guardians",     abbr:"CLE", emoji:"🛡️" }, { id:"115", name:"Colorado Rockies",        abbr:"COL", emoji:"⛰️" },
  { id:"116", name:"Detroit Tigers",          abbr:"DET", emoji:"🐯" }, { id:"117", name:"Houston Astros",          abbr:"HOU", emoji:"🚀" },
  { id:"118", name:"Kansas City Royals",      abbr:"KC",  emoji:"👑" }, { id:"119", name:"Los Angeles Dodgers",     abbr:"LAD", emoji:"💙" },
  { id:"120", name:"Washington Nationals",    abbr:"WSH", emoji:"🦅" }, { id:"121", name:"New York Mets",           abbr:"NYM", emoji:"🗽" },
  { id:"133", name:"Oakland Athletics",       abbr:"OAK", emoji:"🐘" }, { id:"134", name:"Pittsburgh Pirates",      abbr:"PIT", emoji:"☠️" },
  { id:"135", name:"San Diego Padres",        abbr:"SD",  emoji:"🏖️" }, { id:"136", name:"Seattle Mariners",        abbr:"SEA", emoji:"🧭" },
  { id:"137", name:"San Francisco Giants",    abbr:"SF",  emoji:"🌉" }, { id:"138", name:"St. Louis Cardinals",     abbr:"STL", emoji:"🦅" },
  { id:"139", name:"Tampa Bay Rays",          abbr:"TB",  emoji:"☀️" }, { id:"140", name:"Texas Rangers",           abbr:"TEX", emoji:"⭐" },
  { id:"141", name:"Toronto Blue Jays",       abbr:"TOR", emoji:"🦅" }, { id:"142", name:"Minnesota Twins",         abbr:"MIN", emoji:"🌙" },
  { id:"143", name:"Philadelphia Phillies",   abbr:"PHI", emoji:"🔔" }, { id:"144", name:"Atlanta Braves",          abbr:"ATL", emoji:"🪓" },
  { id:"145", name:"Chicago White Sox",       abbr:"CWS", emoji:"⚫" }, { id:"146", name:"Miami Marlins",           abbr:"MIA", emoji:"🐟" },
  { id:"147", name:"New York Yankees",        abbr:"NYY", emoji:"🗽" }, { id:"158", name:"Milwaukee Brewers",       abbr:"MIL", emoji:"🍺" },
];
      ...
      <div style={{ width: "100%", height: "100%", position: "relative", transformStyle: "preserve-3d", transition: "transform .8s cubic-bezier(.4,0,.2,1)", transform: flipped || !faceDown ? "rotateY(180deg)" : "rotateY(0deg)" }}>
        ...
        <div style={{ position: "absolute", inset: 0, backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden", transform: "rotateY(180deg)",
          ...
          <div style={{ padding: "5px 6px", height: "50%", display: "flex", flexDirection: "column", gap: 1, justifyContent: "space-between", zIndex: 2, position: "relative" }}>
            <div style={{ fontSize: size === "sm" ? 8 : 10, fontWeight: 900, fontFamily: "'Orbitron', sans-serif", color: "#E2E8F0", lineHeight: 1.15, overflow: "hidden", WebkitLineClamp: 1, WebkitBoxOrient: "vertical" }}>{play?.playerName || "—"}</div>
            ...
            <div style={{ fontSize: size === "sm" ? 7 : 8, color: "#64748B", lineHeight: 1.3, overflow: "hidden", WebkitLineClamp: size === "xs" ? 1 : 2, WebkitBoxOrient: "vertical" }}>{play?.description || ""}</div>
...
export function PackOpenModal({ pack, plays, onClose, onKeep }) {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      pct += 3;
      setTearPct(pct);
      if (pct >= 100) { clearInterval(t); setTimeout(() => setPhase("fanout"), 300); }
    }, 20);
  ...
  const handleCardClick = (i) => {
    ...
    if (flipped.length + 1 >= plays.length) setTimeout(() => setPhase("done"), 1200);
  };
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 600, overflow: "hidden" }}>
      ...
      {phase === "tear" && (
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20 }}>
          <div style={{ fontSize: mob ? 13 : 18, fontFamily: "'Orbitron', sans-serif", color: "#00D4FF", letterSpacing: ".12em", marginBottom: 10, fontWeight: 900 }}>{pack.emoji} {pack.name.toUpperCase()}</div>
          <div style={{ position: "relative", width: mob ? 160 : 200, height: mob ? 220 : 280 }}>
            <div style={{ width: "100%", height: `${100 - tearPct}%`, background: "linear-gradient(160deg,#0d1f3c,#1a3060)", border: "2px solid rgba(0,212,255,.4)", borderRadius: 14, overflow: "hidden", transition: "height .02s", position: "relative" }}>
              ...
              {tearPct > 5 && <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 3, background: "linear-gradient(90deg,transparent,#00D4FF,transparent)", animation: "shimmer 0.5s linear infinite" }} />}
            </div>
          ...
          <div style={{ fontSize: 11, color: "#334155", fontFamily: "'Orbitron', sans-serif", letterSpacing: ".1em", marginTop: 8 }}>{tearPct < 50 ? "TEARING OPEN..." : tearPct < 90 ? "ALMOST..." : "READY!"}</div>
        </div>
      )}
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start", paddingTop: mob ? 16 : 30 }}>
          <div style={{ textAlign: "center", marginBottom: mob ? 12 : 16, zIndex: 10 }}>
            ...
            {phase === "reveal" && flipped.length < plays.length && (
              <div style={{ fontSize: 10, color: "#334155", marginTop: 4, fontFamily: "'Orbitron', sans-serif" }}>TAP A CARD TO REVEAL · {flipped.length}/{plays.length}</div>
            )}
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: mob ? 8 : 12, justifyContent: "center", maxWidth: mob ? 360 : 760, padding: "0 12px", animation: phase === "fanout" ? "cardFanOut .6s ease both" : "none", flex: 1, alignContent: "center" }}>
            {plays.map((p, i) => {
              ...
              return (
                <div key={i} style={{ position: "relative", animation: phase === "fanout" ? `cardDrop .5s ${i * 0.08}s cubic-bezier(.34,1.56,.64,1) both` : "none", transform: isActive && isFlipped ? "scale(1.12)" : "scale(1)", transition: "transform .3s", zIndex: isActive ? 20 : 10 }}>
                  ...
                  {isFlipped && isActive && (
                    <div key={particleKey} style={{ position: "absolute", inset: 0, pointerEvents: "none", zIndex: 30 }}>
                      {[...Array(16)].map((_, j) => {
                        ...
                        return (
                          <div key={j} style={{ position: "absolute", top: "50%", left: "50%", width: j % 3 === 0 ? 6 : 4, height: j % 3 === 0 ? 6 : 4, borderRadius: "50%", background: rc.color,
                            animation: `burst .6s ${j * 0.02}s ease-out forwards`, "--angle": `${angle}deg`, "--dist": `${dist}px`, transformOrigin: "center" }} />
                        );
                      })}
                    </div>
                  )}
                  {isFlipped && (p.rarity === "legendary" || p.rarity === "epic" || p.rarity === "mystic") && (
                    <div style={{ position: "absolute", inset: -4, borderRadius: 14, border: `2px solid ${rc.color}`, animation: `${rc.anim}`, pointerEvents: "none", zIndex: 5 }} />
                  )}
                </div>
              );
            })}
      ...
      <div style={{ marginBottom: 14 }}><Lbl>Custom Photo</Lbl>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <label style={{ padding: "7px 14px", borderRadius: 8, background: "rgba(0,212,255,.08)", border: "1px solid rgba(0,212,255,.25)", color: "#00D4FF", fontSize: 11, cursor: "pointer", fontFamily: "'Orbitron', sans-serif", fontWeight: 700, flexShrink: 0 }}>
            📷 Upload Photo
            <input type="file" accept="image/*" style={{ display: "none" }} onChange={e => { const file = e.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = (ev) => setCustomHeadshot(ev.target.result); reader.readAsDataURL(file); }} />
          </label>
    ...
    const t = setTimeout(async () => {
      ...
      try {
        ...
        setResults((d.athletes || []).map(a => ({
          ...
          headshot_url: mlbPlayerHeadshot(a.id), cost: 200,
        })));
      } catch (e) { setResults([]); }
      setSearching(false);
    }, 400);
    ...
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 18, flexWrap: "wrap" }}>
        {[["player", "👤 Players", "200 ⭐"], ["team", "🏟️ Teams", "800 ⭐"]].map(([t, l, cost]) => (
          <button key={t} onClick={() => { setType(t); setResults([]); setQ(""); }} style={{ padding: "8px 18px", borderRadius: 10, cursor: "pointer", fontFamily: "'Orbitron', sans-serif", fontSize: 11, fontWeight: 700, border: `1px solid ${type === t ? "rgba(0,212,255,.5)" : "rgba(255,255,255,.1)"}`, background: type === t ? "rgba(0,212,255,.1)" : "rgba(255,255,255,.03)", color: type === t ? "#00D4FF" : "#64748B" }}>{l} <span style={{ fontSize: 9, color: "#334155" }}>{cost}</span></button>
        ))}
      </div>
      {type === "player" && (
        <>
              ...
              return (
                <Card key={card.id} style={{ padding: "14px 12px 16px", textAlign: "center" }}>
                  ...
                  {owned
                    ...
                    : <button onClick={() => onBuy(card)} disabled={!cu || stars < 200} style={{ width: "100%", padding: "8px", borderRadius: 8, background: cu && stars >= 200 ? "rgba(0,212,255,.12)" : "rgba(255,255,255,.04)", border: `1px solid ${cu && stars >= 200 ? "rgba(0,212,255,.35)" : "rgba(255,255,255,.08)"}`, color: cu && stars >= 200 ? "#00D4FF" : "#334155", fontFamily: "'Orbitron', sans-serif", fontSize: 11, fontWeight: 700, cursor: cu && stars >= 200 ? "pointer" : "not-allowed" }}>{!cu ? "Sign in to buy" : "Buy · 200 ⭐"}</button>
                  }
                </Card>
              );
            })}
...
// ----- My Cards Tab (fixed duplicate headshot) -----
        ...
        return (
          <Card key={card.id} style={{ padding: 14, border: isSelected ? "1px solid rgba(0,212,255,.5)" : undefined }}>
            <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
              <CardDisplay type={card.card_type} name={card.card_name} headshot={card.custom_headshot || card.headshot_url} totalRating={card.total_play_rating || 0} customName={card.custom_name || undefined} customBorder={card.custom_border || undefined} customBg={card.custom_bg || undefined} customEffect={card.custom_effect || undefined} size="sm" pinned={card.pinned} serial={card.serial} />
              ...
              <div style={{ marginTop: 12, borderTop: "1px solid rgba(255,255,255,.06)", paddingTop: 12 }}>
                ...
                <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 6 }}>
                  {unapplied.slice(0, 15).map(p => {
                    const pd = typeof p.play_data === "string" ? JSON.parse(p.play_data) : p.play_data;
                    const pr = getPlayRarity(pd?.rating || 0);
                    const prc = RARITY_CFG[pr];
                    return (
...
export function PackShopTab({ cu, stars, loading, onOpen, myTeamCard }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      setSearching(true);
      try {
        const r = await fetch(`/api/hyperbeam?search=${encodeURIComponent(playerSearch)}&sport=mlb`);
        const d = await r.json();
        setPlayerResults(d.athletes || []);
...
export function CollectionTab({ cu, myCards, myPlays }) {
  ...
  const TeamDropdown = ({ value, onChange, style = {} }) => (
    <select value={value} onChange={e => onChange(e.target.value)} style={{ background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.12)", borderRadius: 8, color: "#E2E8F0", fontFamily: "'Rajdhani', sans-serif", fontSize: 13, padding: "8px 12px", cursor: "pointer", flex: "0 0 auto", ...style }}>
  ...
  const OwnTabs = () => (
    <div style={{ display: "flex", gap: 5 }}>
      {[["all", "All"], ["owned", "✅ Owned"], ["unowned", "🔒 Not Owned"]].map(([v, l]) => (
        <button key={v} onClick={() => setOwnFilter(v)} style={{ padding: "6px 12px", borderRadius: 16, cursor: "pointer", fontSize: 10, fontFamily: "'Orbitron', sans-serif", fontWeight: 700, border: `1px solid ${ownFilter === v ? "rgba(0,212,255,.45)" : "rgba(255,255,255,.1)"}`, background: ownFilter === v ? "rgba(0,212,255,.1)" : "rgba(255,255,255,.03)", color: ownFilter === v ? "#00D4FF" : "#64748B" }}>{l}</button>
      ))}
    </div>
  );
  ...
  const filteredPlayerCards = useMemo(() => {
    return myPlayerCards.filter(c => {
      ...
      const teamMatch = !teamFilter || (c.team_name || "").toLowerCase().includes((selectedTeam?.name || "").toLowerCase()) || (c.team_name || "").toLowerCase().includes(teamFilter.toLowerCase());
  ...
  const filteredPlays = useMemo(() => {
    return allPlays.filter(({ pd, owned }) => {
      ...
      const selectedTeam = allTeams.find(t => t.abbr === teamFilter);
      const teamMatch = !teamFilter || (pd.teamName || "").toLowerCase().includes((selectedTeam?.name || teamFilter).toLowerCase()) || (pd.teamName || "").toLowerCase().includes(teamFilter.toLowerCase());
      const ownMatch = ownFilter === "all" || (ownFilter === "owned" && owned) || (ownFilter === "unowned" && !owned);
    ...
    <div>
      <div style={{ display: "flex", gap: 6, marginBottom: 14, flexWrap: "wrap", alignItems: "center" }}>
        {[["players", "👤 Players"], ["teams", "🏟️ Teams"], ["plays", "⚡ Plays"]].map(([s, l]) => (
          <button key={s} onClick={() => { setSection(s); resetFilters(); }} style={{ padding: "7px 16px", borderRadius: 20, cursor: "pointer", fontFamily: "'Orbitron', sans-serif", fontSize: 10, fontWeight: 700, border: `1px solid ${section === s ? "rgba(245,158,11,.5)" : "rgba(255,255,255,.1)"}`, background: section === s ? "rgba(245,158,11,.1)" : "rgba(255,255,255,.03)", color: section === s ? "#F59E0B" : "#64748B" }}>{l}<span style={{ marginLeft: 5, fontSize: 9, color: section === s ? "#F59E0B" : "#334155" }}>{s === "players" ? `${myPlayerCards.length} owned` : s === "teams" ? `${myTeamCards.length}/30` : `${allPlays.length} collected`}</span></button>
        ))}
      </div>
      <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap", alignItems: "center" }}>
        <input value={q} onChange={e => setQ(e.target.value)} placeholder={section === "players" ? "Search player name…" : section === "teams" ? "Search team…" : "Search player or play…"} style={{ flex: 1, minWidth: 160 }} />
        {section !== "teams" && <TeamDropdown value={teamFilter} onChange={setTeamFilter} style={{ minWidth: 140, maxWidth: 200 }} />}
        {(section === "teams" || section === "plays") && <OwnTabs />}
        {hasFilters && <button onClick={resetFilters} style={{ padding: "7px 12px", borderRadius: 8, background: "rgba(239,68,68,.1)", border: "1px solid rgba(239,68,68,.25)", color: "#EF4444", fontSize: 10, cursor: "pointer", fontFamily: "'Orbitron', sans-serif", fontWeight: 700, flexShrink: 0 }}>✕ Clear</button>}
      </div>
      ...
      {section === "players" && (
        <div>
          {!myPlayerCards.length && <div style={{ textAlign: "center", padding: "40px 20px" }}><div style={{ fontSize: 36, marginBottom: 10 }}>👤</div><div style={{ color: "#334155", fontSize: 13, fontFamily: "'Orbitron', sans-serif" }}>No player cards yet</div><div style={{ color: "#1e3a5f", fontSize: 11, marginTop: 4 }}>Head to Market · 200⭐ each</div></div>}
          {myPlayerCards.length > 0 && filteredPlayerCards.length === 0 && <div style={{ textAlign: "center", padding: "30px 20px", color: "#334155", fontFamily: "'Orbitron', sans-serif", fontSize: 11 }}>No cards match these filters</div>}
            ...
            {filteredPlayerCards.map(card => {
              ...
              return (
                <div key={card.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                  <CardDisplay type="player" name={card.card_name} headshot={card.custom_headshot || card.headshot_url} totalRating={card.total_play_rating || 0} customName={card.custom_name || undefined} customBorder={card.custom_border || undefined} customBg={card.custom_bg || undefined} customEffect={card.custom_effect || undefined} size={mob ? "sm" : "md"} pinned={card.pinned} serial={card.serial} />
      ...
      {section === "teams" && (
        <div>
          {filteredTeams.length === 0 && <div style={{ textAlign: "center", padding: "30px 20px", color: "#334155", fontFamily: "'Orbitron', sans-serif", fontSize: 11 }}>No teams match these filters</div>}
          <div style={{ display: "grid", gridTemplateColumns: mob ? "repeat(3,1fr)" : "repeat(auto-fill,minmax(130px,1fr))", gap: mob ? 8 : 10 }}>
            {filteredTeams.map(team => {
              ...
              const rc = RARITY_CFG[rarity];
              return (
                <div key={team.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, opacity: owned ? 1 : 0.32, filter: owned ? "none" : "grayscale(1) brightness(0.4)", transition: "all .2s" }}>
                  {owned
                    ? <CardDisplay type="team" name={team.name} headshot={ownedCard?.custom_headshot || mlbTeamLogo(team.id)} totalRating={ownedCard?.total_play_rating || 0} customBorder={ownedCard?.custom_border || undefined} customEffect={ownedCard?.custom_effect || undefined} size={mob ? "sm" : "md"} pinned={ownedCard?.pinned} serial={ownedCard?.serial} />
                    : <div style={{ width: mob ? 108 : 138, height: mob ? 155 : 198, borderRadius: 12, border: "2px solid rgba(255,255,255,.08)", background: "rgba(255,255,255,.02)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8 }}>
                        <img src={mlbTeamLogo(team.id)} style={{ width: "50%", height: "50%", objectFit: "contain", opacity: .3 }} onError={e => e.target.style.display = "none"} />
                  ...
                  <div style={{ textAlign: "center" }}>
                    <div style={{ fontSize: mob ? 8 : 9, fontFamily: "'Orbitron', sans-serif", color: owned ? rc.color : "#1e3a5f", fontWeight: 700 }}>{owned ? rc.label : "Locked"}</div>
      ...
      {section === "plays" && (
        <div>
          {!allPlays.length && <div style={{ textAlign: "center", padding: "40px 20px" }}><div style={{ fontSize: 36, marginBottom: 10 }}>⚡</div><div style={{ color: "#334155", fontSize: 13, fontFamily: "'Orbitron', sans-serif" }}>No plays collected yet</div><div style={{ color: "#1e3a5f", fontSize: 11, marginTop: 4 }}>Open packs to collect real 2025 MLB play cards</div></div>}
          {allPlays.length > 0 && filteredPlays.length === 0 && <div style={{ textAlign: "center", padding: "30px 20px", color: "#334155", fontFamily: "'Orbitron', sans-serif", fontSize: 11 }}>No plays match these filters</div>}
          <div style={{ display: "flex", flexWrap: "wrap", gap: mob ? 8 : 12 }}>
            {filteredPlays.map(({ pd, owned }, i) => (
              <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5, opacity: owned ? 1 : 0.25, filter: owned ? "none" : "grayscale(1) brightness(0.3)", transition: "all .2s" }}>
                <PlayCard play={pd} size={mob ? "sm" : "md"} />
                {pd.prestige && <div style={{ fontSize: 8, color: "#FFD700", fontFamily: "'Orbitron', sans-serif", fontWeight: 700 }}>✨ PRESTIGE</div>}
                <div style={{ fontSize: 7, color: owned ? "#22C55E" : "#334155", fontFamily: "'Orbitron', sans-serif", fontWeight: 700 }}>{owned ? "OWNED" : ""}</div>
              </div>
            ))}
          </div>
        </div>
