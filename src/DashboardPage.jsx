...
// ─── YouTube / Medal extractors (keep your existing) ─────────────────────────
...
const GM_SPORTS = [
  ...
  { id: "nfl", label: "NFL", icon: "🏈", color: "#F59E0B" },
  { id: "nba", label: "NBA", icon: "🏀", color: "#EC4899" }
];
              ...
              <div style={{ padding: "14px 16px", display: "flex", gap: 12, alignItems: "center" }}>
                ...
                <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
                  ...
                  <button onClick={() => deleteTeam(t.id)} disabled={deleting === t.id}
                    style={{ padding: "5px 8px", borderRadius: 8, background: "rgba(239,68,68,.08)", border: "1px solid rgba(239,68,68,.2)", color: "#EF4444", cursor: "pointer", fontSize: 12, opacity: deleting === t.id ? 0.5 : 1 }}>🗑</button>
                </div>
              </div>
              {tp.length > 0 && (
...
export function DashLeagueRoster({ league, accentColor, cu, sport = "" }) {
  ...
  const AddLeaguePlayer = ({ league, onAdd, cu, sport, leagueTeams }) => {
    ...
    const positions = sport === "basketball" ? basketballPos : (sport === "baseball" ? baseballPos : footballPos);
    const handleAdd = async () => {
      if (!name.trim() || !pos) return;
      setAdding(true);
      const p = { id: gid(), name: name.trim(), position: pos, team: team.trim(), jersey: jersey.trim(), ts: Date.now() };
      await sb.post(`nova_${league}_players`, p);
      onAdd(p);
      setName(""); setPos(""); setTeam(""); setJersey("");
      setAdding(false);
    };
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <input value={team} onChange={e => setTeam(e.target.value)} placeholder="Team (or free agent)" />
        <input value={jersey} onChange={e => setJersey(e.target.value)} placeholder="Jersey #" />
        <Btn onClick={handleAdd} disabled={adding || !name.trim() || !pos}>{adding ? "Adding…" : "➕ Add Player"}</Btn>
      </div>
    );
  };
  return (
    ...
    <Card style={{ padding: "18px" }} hover={false}>
      ...
      <div style={{ display: "flex", gap: 5, marginBottom: 12, flexWrap: "wrap", alignItems: "center" }}>
        {[["season","📅 Season"],["career","🏆 Career"]].map(([t,l]) => (
          <button key={t} onClick={() => { setStatType(t); const ff = t === "season" ? field + "_season" : field; const p = players.find(x => x.id === target); setData(p?.[ff] || {}); }}
      ...
      {showForm && (
        <Card style={{ padding: "16px", marginBottom: 14 }} hover={false}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
            <div><Lbl>Type</Lbl><select value={type} onChange={e => setType(e.target.value)} style={{ width: "100%" }}>{TX_TYPES.map(t => <option key={t}>{t}</option>)}</select></div>
            <div><Lbl>From Player</Lbl><select value={from} onChange={e => setFrom(e.target.value)} style={{ width: "100%" }}><option value="">—</option>{players.map(p => <option key={p.id} value={p.name}>{p.name}</option>)}</select></div>
            <div><Lbl>To / Team</Lbl><select value={to} onChange={e => setTo(e.target.value)} style={{ width: "100%" }}><option value="">—</option>{players.map(p => <option key={p.id} value={p.name}>{p.name}</option>)}</select></div>
            <div style={{ gridColumn: "1/-1" }}><Lbl>Description</Lbl><input value={desc} onChange={e => setDesc(e.target.value)} placeholder="e.g. Aaron Judge traded to Yankees for package…" /></div>
          </div>
...
// ─── DashLeagueMembers (with Roblox avatar fix) ───────────────────────────────
                ...
                <div style={{ display: "flex", alignItems: "flex-end", gap: 14, marginTop: -38, flexWrap: "wrap" }}>
                  <div style={{ position: "relative", width: 92, height: 92, borderRadius: 22, overflow: "hidden", background: "#0F172A", border: `3px solid ${accentColor}`, boxShadow: `0 14px 40px ${accentColor}33`, flexShrink: 0 }}>
                    {loadingRoblox ? (
                      ...
                      <img src={robloxAvatarUrl} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt="roblox avatar" />
                    ) : (
                      <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 38 }}>
                        {sport === "basketball" ? "🏀" : isBaseball ? "⚾" : "🏈"}
                      </div>
                    )}
                  </div>
                ...
                {positions.map(pos => {
                  ...
                  return (
                    <button key={pos} type="button" onClick={() => setAddPos(prev => sel ? prev.filter(x => x !== pos) : [...prev, pos])}
                      style={{ padding: "4px 10px", borderRadius: 8, cursor: "pointer", fontFamily: "'Orbitron',sans-serif", fontSize: 10, fontWeight: 700, border: `1px solid ${sel ? accentColor + "88" : "rgba(255,255,255,.1)"}`, background: sel ? accentColor + "22" : "rgba(255,255,255,.03)", color: sel ? accentColor : "#64748B", transition: "all .15s" }}>
                      {pos}
                    </button>
                  );
                })}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {announcements.map((a, i) => (
                  <Card key={i} style={{ padding: "12px 16px" }}>
                    ...
                    <div style={{ fontSize: 10, color: "#334155" }}>{new Date(a.ts).toLocaleString()}</div>
                  </Card>
                ))}
              </div>
            </div>
