...
// Helper to get Roblox avatar URL
function getRobloxAvatarUrl(userId) {
  if (!userId) return null;
  return `/api/roblox-avatar?userId=${userId}&t=${Date.now()}`;
}
...
export default function MembersPage({ users, nav }) {
  ...
  useEffect(() => {
    users.forEach(user => {
      if (user.social_roblox && !robloxAvatars[user.id]) {
        // Preload avatar by setting a flag; actual image will load via img onError/onLoad
        setRobloxAvatars(prev => ({ ...prev, [user.id]: user.social_roblox }));
      }
    });
  }, [users]);
    ...
    <div style={{ position: "relative", minHeight: "100vh", background: "radial-gradient(circle at 10% 20%, #0B0F19, #030712)", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 2, maxWidth: 1280, margin: "0 auto", padding: "44px 20px 100px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          ...
          {/* Search & filters with modern styling */}
        ...
        <div style={{
          ...
          {filtered.map(user => {
            const hasRoblox = user.social_roblox;
            const avatarUrl = hasRoblox ? getRobloxAvatarUrl(user.social_roblox) : (user.avatar_url || null);
                  ...
                  <div style={{
                    ...
                    {avatarUrl ? (
                      <img
                        ...
                        alt="avatar"
                        onError={(e) => { e.target.style.display = "none"; e.target.parentElement.innerHTML = fallbackEmoji; }}
                      />
                    ) : (
